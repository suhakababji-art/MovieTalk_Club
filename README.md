...
<head>
    ...
    <script type="module">
        ...
        function renderShows(shows) {
            ...
            if (shows.length === 0) {
                list.innerHTML = `<div class="bg-white rounded-3xl p-10 text-center border-2 border-dashed brown-border"><p class="opacity-50 font-bold">لا توجد عروض حالياً..</p></div>`;
                return;
            }
                ...
                <div class="bg-white rounded-3xl shadow-sm overflow-hidden flex flex-col md:flex-row border-2 brown-border hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-8 md:w-2/3 flex flex-col justify-center">
                        <div class="flex flex-wrap gap-2 text-xs font-bold mb-4">
                            <span class="bg-stone-100 px-3 py-1.5 rounded-lg text-stone-600">📅 ${show.date}</span>
                            <span class="bg-stone-100 px-3 py-1.5 rounded-lg text-stone-600">🕒 ${show.time}</span>
                            <span class="bg-teal-50 px-3 py-1.5 rounded-lg teal-text border border-teal-100">📍 ${show.room}</span>
                        </div>
        ...
        window.clearAllData = async () => {
            if (!currentUser || !confirm("حذف كل البيانات؟")) return;
            try {
        ...
        window.generateAIAnalysis = async () => {
            ...
            content.innerHTML = "جاري التحليل...";
            try {
...
<body class="min-h-screen">
    <nav class="teal-bg text-white shadow-lg sticky top-0 z-50 p-4">
        <div class="container mx-auto flex justify-between items-center">
            ...
            <div class="flex gap-4 font-bold text-sm">
                <button onclick="showTab('home')">الرئيسية</button>
                <button onclick="showTab('ai-assistant')" class="text-amber-200">✨ المحلل</button>
                <button onclick="showTab('contact')">المسؤولين</button>
            </div>
        </div>
    </nav>
    ...
    <main class="container mx-auto p-6 max-w-4xl py-12">
        <section id="home" class="tab-content active space-y-8">
            <h3 class="text-3xl font-bold border-r-8 border-[#008080] pr-4">العروض القادمة</h3>
            
            <div id="admin-login-box" class="hidden bg-white p-6 rounded-3xl shadow-xl border-2 border-[#008080] text-center space-y-4">
                <input type="password" id="admin-pass-input" class="p-3 border-2 rounded-xl text-center" placeholder="كلمة السر...">
                <button onclick="verifyPassword()" class="teal-bg text-white px-6 py-3 rounded-xl font-black">دخول</button>
            </div>

            <div id="admin-section" class="hidden admin-panel mb-10">
                <div class="grid gap-4">
                    <input type="text" id="new-title" placeholder="اسم الفيلم (English)" class="p-3 rounded-xl border-2">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input type="text" id="new-date" placeholder="التاريخ" class="p-3 rounded-xl border-2">
                        <input type="text" id="new-time" placeholder="الوقت" class="p-3 rounded-xl border-2">
                        <input type="text" id="new-room" placeholder="رقم القاعة" class="p-3 rounded-xl border-2">
                    </div>
                    <textarea id="new-desc" placeholder="وصف النقاش..." class="p-3 rounded-xl border-2 h-24"></textarea>
                    <button onclick="addNewShow()" class="teal-bg text-white p-4 rounded-xl font-black">نشر للجميع 🚀</button>
                    <button onclick="clearAllData()" class="text-red-500 text-xs mt-4 underline">حذف كل العروض</button>
                </div>
            </div>
        ...
        <section id="ai-assistant" class="tab-content space-y-6 max-w-2xl mx-auto">
            <div class="bg-white p-8 rounded-[2rem] shadow-xl text-center">
                <h3 class="text-2xl font-black teal-text mb-4">🤖 المحلل الذكي</h3>
                <input type="text" id="movie-input" placeholder="Movie Name..." class="w-full p-4 rounded-2xl border-2 mb-4 text-center">
                <button onclick="generateAIAnalysis()" id="ai-btn" class="teal-bg text-white font-bold py-4 px-8 rounded-2xl w-full">تحليل ✨</button>
    ...
    <footer onclick="handleFooterClick()" class="brown-bg text-[#D7CCC8] py-16 mt-20 text-center cursor-pointer">
        ...
        <p class="text-xs font-bold">English Department | © 2026</p>
    </footer>
</body>
</html>
